...
async function fetchAdCopy(discordId1, discordId2) {
  const url = `https://pickandpartnerbackend-titu.onrender.com/api/copyAD?discordId1=${discordId1}&discordId2=${discordId2}`;
...
async function handleSendUtmLinks(client, interaction) {
  try {
    ...
    const response = await fetch(
      "https://pickandpartnerbackend-titu.onrender.com/api/linkverify",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
